...
const REQUEST_BUTTON = { id: "docreq", title: "📥 Get it in minutes" };
const REQUEST_ROW = {
  id: "docreq",
  title: "📥 Request & get in mins",
  description: "Not listed? Request it and get it within a few minutes",
};
const REQUEST_PROMPT =
  "📥 Can't find it? Tap *Get it in minutes* to request it — we'll add it to the HVAC Assistant library and send it to you within a few minutes.";
...
function confirmText(query) {
  return `✅ Request sent — you'll get *"${query}"* within a few minutes. We're adding it to the HVAC Assistant library.`;
}

function duplicateText(query) {
  return `👍 *"${query}"* has already been requested — you'll get it within a few minutes.`;
}

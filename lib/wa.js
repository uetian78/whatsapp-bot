...
// Shared by every "detailed" Bot Sender send (text, document, ...): posts to
// the Graph API and reports what it said instead of a bare boolean — the Bot
// Sender app needs the wamid (to poll delivery status) or the error code to
// show the user.
async function postMessageDetailed(to, payload, label) {
  ...
  try {
    ...
    console.log(`✅ Sent ${label} to ${to} (Bot Sender)`);
...
async function sendTextDetailed(to, body) {
  ...
  return postMessageDetailed(to, payload, "text");
}

// Send an already-uploaded media id as a document, on behalf of the Bot
// Sender app. No caption (unlike sendDocument, which is used by the bot's
// own replies and always adds one).
async function sendDocumentDetailed(to, mediaId, filename) {
  const payload = { messaging_product: "whatsapp", to, type: "document", document: { id: mediaId, filename } };
  return postMessageDetailed(to, payload, "document");
}
...
async function uploadMedia({ link, fileId, filename, exportMime }) {
  const buffer = await downloadBytes({ link, fileId, exportMime });
...
// Upload a file straight from a readable stream (e.g. the raw HTTP request
// body of an Android app upload) to WhatsApp media — never buffered into a
// Buffer, so a 100 MB attachment never sits in Render's memory. knownLength
// lets form-data set Content-Length instead of switching to chunked
// transfer. An optional AbortSignal lets the caller cancel the outbound
// Graph request immediately (e.g. when the uploading client disconnects
// mid-stream) instead of it sitting until the 10-minute timeout. Throws on
// failure (caller maps the error).
async function uploadMediaStream(stream, filename, knownLength, signal) {
  const mime = mimeFromName(filename);
  const form = new FormData();
  form.append("messaging_product", "whatsapp");
  form.append("type", mime);
  form.append("file", stream, { filename, contentType: mime, knownLength });
...
async function sendDriveFile(to, file) {
  ...
  if (isImage) {
    try {
      const mediaId = await uploadMedia({ fileId: file.id, filename: file.name, exportMime: file.exportMime });
  ...
  let buffer;
  try {
    buffer = await downloadBytes({ fileId: file.id, exportMime: file.exportMime });
...
module.exports = {
  send, sendText, sendTextDetailed, sendDocumentDetailed, sendLongText, sendButtons, sendDocument, sendDriveFile, sendPdfBuffer,
  sendList, markReadWithTyping,
  uploadMedia, uploadMediaBuffer, uploadMediaStream, downloadWhatsAppMedia, mimeFromName, validatePdfBuffer,

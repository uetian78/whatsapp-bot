...
function haystack(file) {
  return {
    name: norm((file?.name || "").replace(/\.[^.]+$/, "")),
    folder: norm(file?.folder || ""),
  };
}

// How informative is each query word, judged against this index?
//
// Without this, every matched word scores the same, so "trane" hitting forty
// MTZ documents counts as much as "mcwfa" hitting the one file the user
// actually wants — and the brand-only matches crowd out the right answer.
// A word matching one file in 386 is far more informative than one matching
// forty, so weight by inverse document frequency. Squared, because plain IDF
// leaves a common brand word too close to a distinctive model code.
function tokenWeights(tokens, files) {
  const total = Math.max((files || []).length, 1);
  const weights = new Map();

  for (const raw of tokens) {
    const t = norm(raw);
    if (!t || weights.has(t)) continue;
    let df = 0;
    for (const file of files || []) {
      const h = haystack(file);
      if (h.name.includes(t) || h.folder.includes(t)) df++;
    }
    const idf = Math.log((total + 1) / (df + 1));
    weights.set(t, Math.max(idf, 0.1) ** 2 + 1);
  }
  return weights;
}
...
// `weights` is optional; without it every word counts the same (the old
// behavior), which keeps this usable for a one-off score.
function scoreFile(tokens, file, weights) {
  const { name, folder } = haystack(file);
  let score = 0;
  let matched = 0;
  let weightSum = 0;
  ...
  for (const token of tokens) {
    const t = norm(token);
    if (!t) continue;
    const w = weights ? (weights.get(t) ?? 1) : 1;
    weightSum += w;
    if (name.includes(t)) { score += 3 * w; matched++; }
    else if (folder.includes(t)) { score += 1 * w; matched++; }
  }

  // Whole query appearing intact in the filename is the strongest signal.
  // Scaled by the average word weight so the bonus stays proportionate to
  // the rest of the score rather than swamping it on a rare-word query.
  if (tokens.length > 1 && name.includes(norm(tokens.join("")))) {
    score += 5 * (weightSum / tokens.length || 1);
  }
...
function rankFiles(text, files, limit = 10) {
  ...
  const scored = [];
  for (const file of files || []) {
    const { score, matched } = scoreFile(tokens, file, weights);
...
// ---- Escape hatches, in escalating order of cost ----
// 1. "search all" — free ranked scan of the whole index (rankFiles above).
// 2. "ai search"  — Claude reads the whole file list. Costs a call, so it is
//    only ever OFFERED after the free scan has already failed.
const SEARCH_ALL_HINT =
  "🔍 Not what you wanted? Reply *search all* to search every file in the Drive folder.";
const AI_SEARCH_HINT =
  "🤖 Still not it? Reply *ai search* and I'll have AI look through every file.";

function isSearchAllTrigger(text) {
  return /^\s*search\s+(all|everything|drive)(\s+files?)?\s*$/i.test(String(text ?? ""));
}

function isAiSearchTrigger(text) {
  return /^\s*(ai\s+search|search\s+(with\s+)?ai|deep\s+search|ask\s+ai)\s*$/i.test(String(text ?? ""));
}

module.exports = {
  tokenize, meaningfulTokens, hasSearchableExtras,
  scoreFile, rankFiles, tokenWeights, NOISE,
  isSearchAllTrigger, SEARCH_ALL_HINT,
  isAiSearchTrigger, AI_SEARCH_HINT,
};
